...
async function deleteRecentPosts() {
  try {
    console.log('Searching for posts from the last 6 hours...');
    
    // Calculate the timestamp for 6 hours ago
    const sixHoursAgo = new Date(Date.now() - 6 * 60 * 60 * 1000);
    
    // Find all posts created in the last 6 hours
    const query = {
    ...
    if (postsToDelete.length === 0) {
      console.log('No posts from the last 6 hours found.');
      mongoose.disconnect();
      return;
    }
    
    console.log(`Found ${postsToDelete.length} posts from the last 6 hours to delete.`);
    ...
    console.log(`Successfully deleted ${deleteResult.deletedCount} posts from the last 6 hours.`);
    
    // Display sample of the deleted posts for verification
    console.log('\nSample of deleted posts:');
    postsToDelete.slice(0, 5).forEach((post, index) => {
      console.log(`${index + 1}. Post ID: ${post._id}, User ID: ${post.user}, Created at: ${post.createdAt}, Description: ${post.description ? post.description.substring(0, 30) + '...' : 'No description'}`);
    });
